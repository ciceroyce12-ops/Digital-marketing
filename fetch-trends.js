...
// Using Google News RSS for Indonesia as a reliable, unblocked trends feed
const GOOGLE_NEWS_URL = 'https://news.google.com/rss?hl=id-ID&gl=ID&ceid=ID:id';
...
async function fetchGoogle() {
    try {
        ...
        if (!res.ok) {
            console.error(`Google News RSS returned status ${res.status}`);
            return [];
        }
        ...
        while ((match = itemRegex.exec(xml)) !== null && results.length < 4) {
            ...
            if (titleMatch) {
                ...
                results.push({
                    ...
                    traffic: 'Terbaru',
                    started: 'Hari ini',
                    url: linkMatch ? linkMatch[1].trim() : 'https://news.google.com'
                });
            }
        }
...
async function run() {
    ...
    console.log("trends.json successfully generated with live news data!");
}
